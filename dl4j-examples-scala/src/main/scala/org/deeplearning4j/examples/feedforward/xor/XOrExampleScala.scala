 ...
 *
 * @author Romeo Kienzler (ported from Peter Großmann's Java implementation)
 */
object XOrExampleScala {
  def main(args: Array[String]): Unit = {
    ...
    /*
     *  correspondending list with expected output values, 4 training samples with
     ...
     *  1000, or a higher learning-rate - found this values just by trial and error
     */
     ...
     * the previous layers finetuned weights that's for autoencoders and rbms
     */
    listBuilder.backprop(true);

    /*
     * build and init the network, will check if everything is configured correct
     */
     ...
     */
    var layers: Array[Layer] = net.getLayers();
    var totalNumParams = 0;
    var i = 0
    while (i < layers.length) {
      var nParams = layers(i).numParams();
     ...
     */
    var output: INDArray = net.output(ds.getFeatureMatrix());
    println(output);
